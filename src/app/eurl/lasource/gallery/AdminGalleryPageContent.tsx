...
export default function AdminGalleryPage() {
  ...
  // Delete confirmation
  const [deleteTarget, setDeleteTarget] = useState<{ urls: string[]; names: string[] } | null>(null);
  ...
  const selectAll = () => {
    if (selectedFiles.size === paginatedFiles.length) {
      ...
      setSelectedFiles(new Set(paginatedFiles.map(f => f.url)));
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch('/api/gallery', {
        ...
        body: JSON.stringify({ urls: deleteTarget.urls }),
      });
      if (res.ok) {
        setSelectedFiles(new Set());
        fetchFiles();
    ...
    <div className="space-y-6">
          ...
          {selectedFiles.size > 0 && (
            <div className="flex items-center gap-1.5">
              ...
              <Button
                ...
                onClick={() => {
                  const sel = files.filter(f => selectedFiles.has(f.url));
                  setDeleteTarget({
                    urls: sel.map(f => f.url),
                    names: sel.map(f => f.fileName),
                  });
                }}
              >
      ...
      {isSelecting && paginatedFiles.length > 0 && (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <button onClick={selectAll} className="flex items-center gap-1.5 hover:text-foreground cursor-pointer">
            {selectedFiles.size === paginatedFiles.length ? (
              <CheckSquare className="h-4 w-4 text-primary" />
            ) : (
              <Square className="h-4 w-4" />
            )}
            Select all {paginatedFiles.length} on this page
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          {!searchQuery && folderFilter === 'all' && typeFilter === 'all' && (
            <Button className="mt-4 cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                ...
                {isSelecting && (
                  ...
                  >
                    {selectedFiles.has(file.url) ? (
                      ...
                      <Square className="h-5 w-5 text-muted-foreground bg-background/80 rounded" />
                    )}
                  </button>
                )}
                  ...
                  <div className="relative aspect-square bg-muted">
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button
                              ...
                              className="h-8 w-8 rounded-full bg-white/90 text-gray-800 flex items-center justify-center cursor-pointer hover:bg-white"
                            >
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>{copiedUrl === file.url ? 'Copied!' : 'Copy URL'}</TooltipContent>
                      </Tooltip>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              setDeleteTarget({ urls: [file.url], names: [file.fileName] });
                            }}
                            className="h-8 w-8 rounded-full bg-red-500/90 text-white flex items-center justify-center cursor-pointer hover:bg-red-600"
                          >
          ...
          <CardContent className="p-0">
            <div className="divide-y">
              ...
              {paginatedFiles.map((file) => (
                <div
                  key={file.url}
                  className={`grid grid-cols-12 gap-2 px-4 py-2.5 items-center text-sm hover:bg-muted/30 transition-colors cursor-pointer ${
                    selectedFiles.has(file.url) ? 'bg-primary/5' : ''
                  }`}
                  onClick={() => {
                    if (isSelecting) toggleSelect(file.url);
                    else if (file.type === 'image') setPreviewFile(file);
                  }}
                >
                  <div className="col-span-1">
                    {isSelecting && (
                      <button onClick={(e) => { e.stopPropagation(); toggleSelect(file.url); }} className="cursor-pointer">
                        {selectedFiles.has(file.url) ? (
                          <CheckSquare className="h-4 w-4 text-primary" />
                        ) : (
                  ...
                  <div className="col-span-5 flex items-center gap-2 min-w-0">
                    ...
                    <span className="truncate text-xs" title={file.fileName}>{file.fileName}</span>
                  </div>
                  <div className="col-span-2">
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4">{file.folder}</Badge>
                  </div>
            ...
            <>
              <DialogHeader className="p-4 pb-0">
                ...
                <DialogDescription className="text-xs">
                  {previewFile.folder} &middot; {previewFile.sizeFormatted} &middot; {previewFile.mimeType}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="flex items-center justify-between p-4 pt-2">
                ...
                <div className="flex gap-2 flex-shrink-0">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setDeleteTarget({ urls: [previewFile.url], names: [previewFile.fileName] });
                      setPreviewFile(null);
                    }}
                  >
                    <Trash2 className="h-3.5 w-3.5 mr-1" />
      ...
      <Dialog open={!!deleteTarget} onOpenChange={() => setDeleteTarget(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteTarget && deleteTarget.urls.length === 1
                ? 'Are you sure you want to delete this file? This action cannot be undone.'
                : `Are you sure you want to delete ${deleteTarget?.urls.length} files? This action cannot be undone.`}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button variant="destructive" onClick={handleDelete} className="cursor-pointer">
              ...
              Delete {deleteTarget?.urls.length === 1 ? 'File' : `(${deleteTarget?.urls.length})`}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
